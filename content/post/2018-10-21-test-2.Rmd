...
[Barbados](https://www.visitbarbados.org/) is hot. It's a tropical paradise in the Caribbean it was always hot but these days, thanks to [global warming](https://www.nationalgeographic.com/environment/global-warming/global-warming-overview/?user.testname=none), it is hotter than ever before. Exactly how hot is it recently?

```{r echo=FALSE}

blogdown::shortcode('tweet', '1022169368300376064')
```
That sums it up perfectly, the question isn't if Barbados is getting warmer, that's a  definite. What I really wanted to know was what does warmer looks like and so I built some charts. 

The data manipulation and visualisation were done using [R](https://www.r-project.org/) and you can find the source data [here](https://gist.githubusercontent.com/jamaalroach/bc8e31c8756813ad485ec100d921dd25/raw/19d08c5a991afa9ad338ba0183fb6720faa9b5be/temp.csv)  and the code [here](https://github.com/jamaalroach/jamaalroach.com2/blob/master/content/post/2018-10-21-test-2.Rmd).

#The Data

Barbados' monthly average temperature from 1901 -2015 was available from the [The World Bank Climate Knowledge Portal](http://sdwebx.worldbank.org/climateportal/index.cfm?page=country_historical_climate&ThisRegion=Latin%20America&ThisCcode=BRB). This was then used to calculate averages annually and over each decade in the period.
...
This chart compares the the range between the minimum aand maximum annual average temperatures during each decade. 
...
```{r}
ggplot(dec, aes(y=rev(dec$decade)))+geom_text(aes( x=dec$mintemp, y=rev(dec$decade), label=dec$decade, hjust=1.40, ))+geom_point(aes(x=dec$mintemp, colour="steelblue", size=1))+geom_point(aes(x=dec$maxtemp, colour="firebrick1", size=1))+geom_segment(x=dec$mintemp, xend=dec$maxtemp, y=rev(dec$decade), yend=rev(dec$decade), colour="gray")+theme(legend.position = "none", panel.background = element_rect(fill="white"), axis.title.y=element_blank(),axis.text.y=element_blank(),axis.ticks.y=element_blank())+ggtitle("Decade Min-Max Range - Average Annual Temperature (°C)")+theme(plot.title = element_text(colour = "gray59", hjust = .5), axis.text.x = element_text(colour = "gray59"), axis.ticks.x = element_blank(),axis.title.x = element_blank())+coord_cartesian(xlim = c(25.3,27.25))+geom_vline(xintercept = 26.5, linetype=3, colour ="gray59")
```
...
For the chart above the baseline used was the `r ltavg` which was average annual temperature over the 20th Century (1901-2000). Again the pattern of increasing temperature is seen with a consistent positive anomalies occuring after 1987. 
...
Each stripe represents a year with the colours being representative of the annual avearge temperature.It is an elegant way to show the  increase over time using just a colour scale.
